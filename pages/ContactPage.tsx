...
const ContactPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setStatus('error');
            setResponseMessage('Sorry, there was an error sending your message. Please try again later.');
        }
    };
                        ...
                        <div className="bg-[var(--c-surface)] p-8 rounded-lg shadow-lg border border-[var(--c-border)]">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div>
                                    ...
                                    <input type="text" id="name" name="name" value={formData.name} onChange={handleChange} required className="mt-1 block w-full px-3 py-2 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-md shadow-sm focus:outline-none focus:ring-[var(--c-accent-primary)] focus:border-[var(--c-accent-primary)]" />
                                </div>
                                <div>
                                    <label htmlFor="email" className="block text-sm font-medium text-[var(--c-text-primary)]/90">Email</label>
                                    <input type="email" id="email" name="email" value={formData.email} onChange={handleChange} required className="mt-1 block w-full px-3 py-2 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-md shadow-sm focus:outline-none focus:ring-[var(--c-accent-primary)] focus:border-[var(--c-accent-primary)]" />
                                </div>
                                <div>
                                    <label htmlFor="subject" className="block text-sm font-medium text-[var(--c-text-primary)]/90">Subject</label>
                                    <input type="text" id="subject" name="subject" value={formData.subject} onChange={handleChange} required className="mt-1 block w-full px-3 py-2 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-md shadow-sm focus:outline-none focus:ring-[var(--c-accent-primary)] focus:border-[var(--c-accent-primary)]" />
                                </div>
                                <div>
                                    <label htmlFor="message" className="block text-sm font-medium text-[var(--c-text-primary)]/90">Message</label>
                                    <textarea id="message" name="message" value={formData.message} onChange={handleChange} rows={5} required className="mt-1 block w-full px-3 py-2 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-md shadow-sm focus:outline-none focus:ring-[var(--c-accent-primary)] focus:border-[var(--c-accent-primary)]"></textarea>
                                </div>
                                <div>
                                    <button type="submit" disabled={status === 'sending'} className="w-full btn-primary text-white font-bold py-3 px-4 rounded-lg shadow-md transition-all disabled:bg-gray-400 disabled:cursor-not-allowed">
                                        {status === 'sending' ? 'Sending...' : 'Send Message'}
                                    </button>
                                </div>
                                {responseMessage && (
                                    <p className={`mt-4 text-center text-sm font-semibold ${status === 'success' ? 'text-green-700' : 'text-red-600'}`}>
                                        {responseMessage}
                                    </p>
                                )}
                            </form>
